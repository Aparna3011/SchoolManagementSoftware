...
export default function Promotion() {
  ...
  const [targets, setTargets] = useState({ targetClass: null, targetYear: null });
  ...
  const markAllPromoted = () => {
    setStudents(prev => prev.map(s => ({ ...s, promotion_status: "Promoted" })));
  };
  ...
  async function handleSave() {
    ...
    if (!targets.targetYear || !targets.targetClass) {
      toast.error("Target academic year or class not found for promotion");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        targetClassId: targets.targetClass.id,
        promotions,
      };
      ...
      toast.error(error.message || "An unexpected error occurred while saving promotions");
    }
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Promote students to the next academic year and class</p>
        </div>
      </div>
        ...
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Select
              ...
              onChange={(e) => setSelectedYear(years.find(y => y.id == e.target.value))}
